...
def solver(X, t0, Efield, Bfield, mass_of_particle): # X contains x,y,z and dx,dy,dz , 6 elements
    """
    PURPOSE:
    --------
    ...
    ax = ELECTRON_CHARGE * (Efield[0] + (vy * Bfield[2]) - (vz * Bfield[1]) ) /mass_of_particle # Lorentz force / mass
    ay = ELECTRON_CHARGE * (Efield[1] - (vx * Bfield[2]) + (vz * Bfield[0]) ) /mass_of_particle
